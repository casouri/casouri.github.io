...
The previous font I created has some problems, it doesn’t contain punctuation marks and misses some common glyphs. I fixed those issues and the new file is at [[../../misc/常用字及符号.txt][here]]. I also don’t want the font file to contain ASCII characters, so that Latin characters can fall back to my English font. So I also changed the command:
