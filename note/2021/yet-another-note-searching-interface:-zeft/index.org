...
Once we have a list of matched files, Zeft goes through each of them and insert the title and an excerpt into the Zeft buffer, and make the whole thing a giant button.

** Need for speed

Zeft uses a few tricks to make searching and updating snappier. First, it only searches when the search term is at least three characters long, because one or two characters usually matches a bunch of files. (But you can always force a search with =C-c C-g=.)

The second trick is borrowed from Deft: when the user inserts the search phrase, the phrase gets longer incrementally and Zeft needs to update the result of each phrase. Observe that if the new search phrase is just the previous search phrase plus a character, files that match the new phrase must match the old phrase. In other words, the new matched file list is a subset of the old list. Therefore, instead of matching in all the files every time, we can search only in previously matched files. On top of that, if the previous matched list has fewer than 50 files, Zeft skips the grep search and just searches in Emacs.

** User interaction details

As in other packages I wrote, I try to get the details right and make sure Zeft suits my habits. For example, because I hate dangling buffers and windows that requires manual cleanup, when Zeft quits, I want it to kill all buffers and windows it created. I got an imitation of that by window configurations: when Zeft starts up, it saves the current window configuration, and when the Zeft buffer is killed, Zeft restores that window configuration. This is easier than tracking all the buffers and windows Zeft created and deciding whether to kill or bury each buffer and whether to delete each window.
