...
我们只处理连续两个可挤压标点的情况，连续三个不处理，居中的标点也不考虑。连续两个可挤压标点的时候有两种处理方法：挤压左边的，或者挤压右边的。如果左边的标点是靠左的类型，那就挤压左边的标点。比如“◊(code "》）")”，左边的右书名号靠左，所以挤压右书名号，把它右边的空白挤掉。如果右边的标点是靠右的类型，挤压右边地标点。比如“◊(code "（《")”，右边的左书名号靠右，那就挤压它，把左边的空白挤掉。如果左边标点靠左，右边标点靠右（◊(code "》《")），挤压哪个都可以；如果左边标点靠右，右边标点靠左（◊(code "《》")），那就不挤压。

前面我说连续三个标点不处理，但也不是完全不处理，而是这样：如果遇到一个可挤压标点，看看后面那个是不是标点，如果是的话按上面的方法挤压。这样一来，如果有连续三个标点，前面两个会正常挤压，后面那个就被无视了，因为它后面没有标点。连续四个的话就会前面两个一起挤压，后面两个一起挤压。三个标点的时候，我可以用零距空格控制挤压前面两个还是后面两个：如果在第一个标点后面插一个零距空格，算法就会挤压后面两个标点。
