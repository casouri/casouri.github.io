...
If I’m behind a ◊sc{nat} or even multiple ◊sc{nat}’s, what happens is that my packets get relayed by my ◊sc{nat}’s and they appears on the public Internet at the out-most ◊sc{nat}’s address with a port assigned to me. And reply packets going to that address:port are relayed back to me. So essentially I got a public address:port that can reach me on the public Internet. The purpose of ◊sc{nat} traversal is to find that public address:port. That’s basically what the initial/classic ◊sc{stun} (◊link["https://datatracker.ietf.org/doc/html/rfc3489"]{◊scom{rfc 3489}}) does.

The truly host-to-host traversal is just that, finding the public address:port. Sometimes having that address:port is not enough, because ◊sc{nat} poses ◊fnref["extra"]{extra restrictions}. Then we have to resort to having a public-visible relay server in the middle, which is what ◊sc{turn} (◊link["https://datatracker.ietf.org/doc/html/rfc5766"]{◊scom{rfc 5766}}) does. But any host-to-host traversal is just finding public address:port. There is no extra magic there.

How does the classic ◊sc{stun} works is quite simple. You send a ◊sc{stun} server a message, the ◊sc{stun} server looks at the source IP address and port of the IP packet, and reply that back to you. Voilà, you know you public ◊code{address:port}!

That leaves us with ◊sc{ice}, what does it do? Basically, people realize that there are so many network environments that are wildly different, so there is no one single method that guarantees to work everywhere. It is best to try a bunch of ways and pick the one that works and works best.

◊sc{ice} does basically that. It gathers a bunch of ◊code{address:port}’s that possibly works (through ◊sc{stun} messages with stun servers) and tries them one-by-one according to some algorithm, and reports to you the best one. If none works, it tries to establish a relay through ◊sc{turn}.
