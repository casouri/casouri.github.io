...
A huge disclaimer upfront: I’m not an expert, only someone that read all of papers and blog posts that I can find, so take what I say with a grain of salt. There’s frustratingly lack of materials of ◊sc{ot} online (which is part of what prompted this article), so I can’t say I’ve grasped the whole picture regarding ◊sc{ot}. If there’s any mistake, or anything I missed, please do let me know. Thanks!
...
Also, since ◊sc{crdt} lives in a different world that what user and editor sees, operations that ◊sc{crdt} produce might not be “intuitive” to the user, even though the document ends up in a consistent state. A big part of collaborative editing is capturing and preserving “user intent”, and it’s harder to do when your core operations are slightly different from what user actually does, and you see a different document internally than what user sees.

There’re ◊fnref["tricks"]{tricks} that we can do to speed up the translation, but it’s still a lot of complexity. On the other hand, a basic ◊sc{ot} algorithm is much easier to implement.

Even though ◊sc{ot} ◊sc{do} is simple, ◊sc{ot} ◊sc{undo} is very complicated and inefficient. We’ll expand on this later sections. Undo ◊sc{crdt} is simple and can be handled as normal operations.

I hope I’ve demonstrated that neither ◊sc{ot} or ◊sc{crdt} is strictly superior to another. Some further comparison can be found in Appendix A.
...
◊scom{tp2} involves three operations, given any operation ◊code{o3}, you should be able to transform it against ◊code{[o1, o2’]}, or against ◊code{[o2, o1’]}, and the result ◊code{o3’} should be the same. This is very hard to achive. But luckly, you only need ◊scom{tp2} if your algorithm needs to transform operations that are not created on the same context, and most algorithms simply arrange the operations so they never to this transformation, instead, they pre-transform the operations so that they’re based on the same context, then transform them against each other. Indeed, how to correctly and efficiently track and store the context of each operation, and find the operation with the right context, etc, would be the job of the control algorithm.

◊;section{State matrix}

◊section{Control algorithm}

The principle of a control algorithm is simple, suppose there’re two editors, and you’re the control algorithm on ◊om{editor 2}, user has made a series of edits ◊code{o1,...,o3}, and in comes a op ◊code{o8}, from ◊om{editor 2}. Op ◊code{o8} is made on a document state different from the document state currently in your editor, so it can’t by applied verbatim. Let’s suppose our transformation function satisfies ◊scom{tp1} but not ◊scom{tp2}, which is typical, how do you transform ◊code{o8} so that it avoids ◊scom{tp2} and the result can apply cleanly on the current document?

Note that a document state is simply made of all the ops applied to it. So we can define the document state, aka the context of any op generate on that state, as a set of ops. So our current document state is ◊code{{o1, o2, o3}}. Conveniently, the three ops are generated by the user sequentially, so ◊code{o1}’s context is ◊code{{}}, ◊code{o2}’s context is ◊code{{o1}}, and ◊code{o3}’s context is ◊code{{o1, o2}}, each can be directly applied in the order of ◊code{o1}, ◊code{o2}, ◊code{o3}.

Now if we look back at ◊code{o8}, how we transform it depends on its context. Suppose by the time ◊om{editor 2} generates ◊code{o8}, it has already received o1, then the context of ◊code{o8} is ◊code{{o1}}. We just need to transform it in some way such that ◊code{o8}’s context becomes ◊code{{o1, o2, o3}} to apply it on our document. And with our current history, there is a valid path to do it: first transform ◊code{o8} against ◊code{o2}, since both have context ◊code{{o1}}, to get ◊code{o8’}. Note that now ◊code{o8’} has context ◊code{{o1, o2}}. Then transform ◊code{o8’} against ◊code{o3} and get ◊code{o8”} which has context ◊code{{o1, o2, o3}}, and it can be applied onto our document. Remember, because our transformation function doesn’t satisfy ◊scom{tp2}, we can only transform an op against another when they have the same context.

So that’s the rough idea, in the real algorithms there’re many ways to play with the context. You can enforce a global sequence so that the context is implied by the position of an op in the sequence. Or you might find yourself with an op with a context that you can’t transform (maybe ◊code{o8}’s context is ◊code{{o1, o3}}, how do you transform that?)
...
On surface, undo with ◊sc{ot} seems simple: for an op ◊code{o}, you just generate the inverse of it ◊code{I(o)}, and treat it as a concurrent operation after o, transform it against all ops after o and apply it on the document. But this won’t work. Suppose you insert "AAA", undo it, then redo the undo. Now the history consist of ◊code{[ins(0, "AAA"), del(0, "AAA"), ins(0, "AAA")]}. Let’s try to undo the first insertion—make a inverse of it, which is ◊code{del(0, "AAA")}; transform it against the second op, now you get ◊code{ins(0, "")}; then you transform it against the third op, of course, nothing changes, and you still get ◊code{ins(0, "")}. That’s wrong! We expect to get ◊code{del(0, "AAA")}. Our transformation function doesn’t know that the third op undoes the second op, so after the content in our op was removed in the first transformation, it didn’t get added back during the second transformation.

What I just described is basically ◊scom{ip2}: transforming an op ◊code{o1} against another op ◊code{o2} followed by it’s inverse ◊code{I(o2)} should give you ◊code{o1} back.

In total, there are three inverse properties ◊scom{ip1}, ◊scom{ip2}, ◊scom{ip3}. ◊scom{ip1} is basic and like ◊scom{tp1}, is always satisfied. It says applying an op followd by its inverse gives you the same document.

◊scom{ip3} says given two concurrent ops ◊code{o1} and ◊code{o2}, if you transform ◊code{o2} against ◊code{o1} to get ◊code{o2’} and apply to the document, then generate an inverse of ◊code{o1} and transform it against ◊code{o2’} to get ◊code{I(o1)’}. This should be the same as transforming o1 against ◊code{o2} to get ◊code{o1’}, then take the inverse of it to get ◊code{I(o1’)}.

Again, ◊scom{ip2} and ◊scom{ip3} are generally avoided with control algorithms. Earlier ◊sc{ot} algorithms tries to associate undo with their original op and skips them in pairs when transforming the inverse op. Later ◊sc{ot} algorithm like ◊sc{cot} uses increasingly complicated and expensive procedure to generate inverse operations.
...
If you want to learn the proper way to do undo with ◊sc{ot}, I recommend reading the ◊fnref["COT"]{◊sc{cot} paper}. If I remember correctly, it’s undo is exponential time.

All in all, I don’t think undo is “solved”. Even Google Doc has cracks. Try this: open the same doc in two tabs, in one tabs type AABB, in the other insert XX between AA and BB. Now undo in both tabs. Redo in the first tab, you get AABB back. Now redo in the second tab, you get XXAABB.
...
There needs to be a central server that receives op from each site, transform them and broadcast the op to each site. Each op’s context is represented by a sequence number in a global sequence, its context is basically all the ops before it combined. We make sure all the ops are generated from/transformed to have this linear, continuous context. This is key to simplify the control algorithm.

The server’s job is simple, it maintains the global history, and receives ops from each site. The ops sent from each site doesn’t always have the latest context, if the current global history is ◊code{[o1,...,o9]}, a site might send a op with context ◊code{[o1,...,o6]}. That’s fine, server just transforms it against ◊code{o7,...,o9}, stores it in the global history, and broadcast it out. The important thing is that client’s op’s context is always a prefix of the global history. Eg, a context like ◊code{[o1, o2, o5, o6]} would never happen.

On the editor side, we maintain a history made of two segments, the global history ◊code{L1} and local history ◊code{L2}. And the current editor document state is at the end of ◊code{L1 + L2}. ◊code{L1} contains all the ops sent from the server, and ◊code{L2} contains all the ops user created that hasn’t been sent to the server yet. If user keeps adding ops, we put them at the end of ◊code{L2}. The ops we receive from the server always have continues global sequence number, and the context of a new op from server is always all the ops in ◊code{L1}. That means whenever a op comes from server, we just need to transform it against all the ops in ◊code{L2} sequentially, and it’ll have the correct context to be applied on the current local document. Also the untransformed server op is appended to the end of ◊code{L1}.

To send ops to server, we always send the whole ◊code{L2} to server together, because ops in ◊code{L2} have continuous context. We send ◊code{L2} to server, server transforms them against the global history it has, and broadcast it out. Note that server can always transform the ◊code{L2} we send out, because ◊code{L2}’s context is ◊code{L1}, which is always a prefix of the global history that server maintains. Then, we received ◊code{L2’} back from server, which is the transformed version of ◊code{L2}, and append it to the end of ◊code{L1}. We also remove the ops from ◊code{L2} since they’re now in ◊code{L1}. Waiting for this ◊sc{ack} from server ensures that our local ◊code{L1} is always a prefix of the global history on the server.

In case of disconnection and lost messages, a site can simply re-request ops from the server and resend it’s local ops, there’s no special synchronization or fixup required.
...
For ◊sc{undo}, I used tomestone. Now I need to translate internal doc position (which counts tombstones) and external editor document position. This is still vastly simpler than trying to use control algorithm to avoid ◊scom{ip2} and ◊scom{ip3}. If someone has a good way to implement undo without tombstones, I’m eager to hear.
...
Time complexity: it various greatly depending on algorithm, optimization, etc. Suffice to say both can be efficient and it really depends.

Finally, there is a ◊fnref["real-difference"]{detailed paper} that compares ◊sc{ot} and ◊sc{crdt}. It was later expanded into a three-part series: ◊fnref["real-difference-1"]{Ⅰ}, ◊fnref["real-difference-2"]{Ⅱ}, ◊fnref["real-difference-3"]{Ⅲ}. The main idea is that ◊sc{crdt} in its essence is still based on transformations, and there are a lot of hidden complications applying the algorithm on paper to editors. The paper makes ◊sc{crdt} sound like being inferior to ◊sc{ot}, but the bias towards ◊sc{ot} is pretty palpable, so I’d take it with a grain of salt ◊smile{}
