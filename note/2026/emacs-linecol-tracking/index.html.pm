...
Handling all the edge cases correctly requires a bit of patience. Harder still is to find a way to use existing functions (◊code{display_count_lines}) to implement the scanning instead of writing a simple scanner to count newlines, because I had to countermand a specific cases where ◊code{display_count_lines}’s behavior doesn’t match what we want (see ◊link["https://github.com/emacs-mirror/emacs/blob/6c79b6bede776d987a228f34a1a33f4fecd03845/src/treesit.c#L1030"]{◊code{treesit-count-lines}}). But who knows what crazy edge case would come up when scanning the buffer, I’d rather hide behind ◊code{display_count_lines} and bear with its quirks.

We also need a function that, given the old linecol of a position, calculate the new linecol of a position after a buffer change. Without it, we’d have to update the linecol of visi_beg and visi_end by scanning from point every time, defeating the usefulness of the cache entirely.
...
Remember the lifecycle of parser updates for tree-sitter in Emacs: for each user edit, we report the change to each parser but don’t reparse yet. Only when the user requests nodes from the parser do we syncup the visible beg and end of the parser with the visible portion of the buffer, and reparse.
