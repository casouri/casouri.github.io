...
(setq wl-draft-config-alist
      '(((string-match "psu" wl-draft-parent-folder)
        ...
        ((string-match "gmail" wl-draft-parent-folder)
         (template . "GMAIL"))))
;; Set config when start draft instead of sending time
(add-hook 'wl-draft-mode-hook #'wl-draft-config-exec)
#+END_SRC

This makes sure I always reply with the address to which the mail was send.
I add the function to =wl-draft-mode-hook= to apply the change as soon as I open a message
buffer, instead of when the mail is sent out.
